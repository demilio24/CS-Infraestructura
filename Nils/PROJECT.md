...
The homepage (`Nils/website/` — note this is the multi-page site, not `funnel/`) was built as a no-scroll single-screen "router page": `home.css` pinned `html, body` to `100svh` with `overflow: hidden` on mobile, hid the hero lead paragraph, and force-split the two offer cards into `1fr 1fr` to fit one viewport. On real iPhones the hero + blue ("Need more clients?") + green ("Drowning in manual work?") cards are taller than the screen, so the green card's CTA was clipped by the Safari bottom bar and **unreachable** (a prior `svh`/`safe-area` tweak didn't help because the real blocker was `overflow: hidden`). Switched the `@media (max-width: 720px)` block to natural flow: `height: auto` + `min-height: 100svh`, `overflow-y: auto`, body grid rows `auto auto`. Then a spacing pass: removed the `#offers` 48px vertical padding so the blue/green cards are **full-bleed** (no white strips around them), and set each `.route-half` to `min-height: 40svh` + `--space-7` vertical padding so they're equal-height, content-driven sections instead of half-empty 50svh blocks. Desktop 50/50 grid (`min-width: 721px`) untouched. Final cache-buster `home.css?v=20260601i`. Verified via Puppeteer at 390×740: page 808px tall (was 1052), scrolls, both CTAs reachable, no white gaps.
